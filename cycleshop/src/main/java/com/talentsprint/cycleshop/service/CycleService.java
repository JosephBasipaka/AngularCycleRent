...
public class CycleService {
    ...
    public void rentCycle(long id, int count) {
        var cycle = findByIdOrThrow404(id);
        if (cycle.getNumAvailable() >= count) {
            cycle.setNumBorrowed(cycle.getNumBorrowed() + count);
            cycleRepository.save(cycle);
        } else {
            throw new CycleShopBusinessException("Not enough available cycles to rent.");
        }
    }

    public void rentCycle(long id) {
        rentCycle(id, 1);
    }
}
